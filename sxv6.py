...
try:
    ...
    print("[YOLO] Successfully imported YOLO")
except ImportError as e:
    print(f"[YOLO] Error importing YOLO: {str(e)}")
    YOLO = None
...
try:
    ...
    print("Warning: picamera2 not found. Will try fallback to regular OpenCV capture.")
...
class CameraThread(QThread):
    ...
    def __init__(self):
        ...
        self.STATE_IDLE = 0  # or whatever default
        
        # Initialize torch and CUDA settings
        try:
            import torch
            ...
            print("[CameraThread] Warning: Could not configure PyTorch:", str(e))

        self.picam2 = None
        self.cap = None
        
        print("[CameraThread] Attempting to initialize camera...")
        self.person_currently_detected = False  # Track current stable detection state for UI mode switching
        
        # First try picamera2 for Raspberry Pi Camera Module 3
        try:
            from picamera2 import Picamera2
            print("[CameraThread] Found picamera2, attempting to initialize Pi Camera...")
            ...
            print("[CameraThread] Successfully initialized Pi Camera with picamera2")
            USING_PI_CAMERA = True
            return  # Successfully initialized Pi Camera, no need to try OpenCV
        except Exception as e:
            print(f"[CameraThread] Error initializing Pi Camera with picamera2: {str(e)}")
            self.picam2 = None
            USING_PI_CAMERA = False
        ...
        if not USING_PI_CAMERA:
            try:
                ...
                print(f"[CameraThread] Error initializing Pi Camera: {str(e)}")
                self.picam2 = None
        
        try:
            self.cap = cv2.VideoCapture(0)  # Try default first
            if not self.cap.isOpened():
                print("[CameraThread] Default initialization failed, trying V4L2...")
                self.cap = cv2.VideoCapture(0, cv2.CAP_V4L2)
            
            if self.cap.isOpened():
                # Configure for Pi Camera Module 3
                self.cap.set(cv2.CAP_PROP_CONVERT_RGB, 1)
                self.cap.set(cv2.CAP_PROP_BUFFERSIZE, 1)
                
                # Set format to MJPG for better performance
                ...
                print(f"[CameraThread] Camera initialized successfully: {actual_width}x{actual_height} @ {actual_fps}fps")
                
                # Read a test frame to ensure camera is working
                ret, _ = self.cap.read()
                if not ret:
                    print("[CameraThread] Warning: Could not read test frame")
                    self.cap.release()
                    self.cap = None
            else:
                print("[CameraThread] Error: Could not open camera with any method")
                self.cap = None
                
        except Exception as e:
            print(f"[CameraThread] Error initializing camera: {str(e)}")
            if self.cap:
                self.cap.release()
            self.cap = None
        
        try:
            # Define possible camera configurations
            camera_configs = [
                (0, cv2.CAP_V4L2),  # Try V4L2 first
                (0, cv2.CAP_ANY),   # Then try default
                (2, cv2.CAP_V4L2),  # Try additional video devices
                (1, cv2.CAP_V4L2)
            ]
            
            success = False
            for device, api in camera_configs:
                try:
                    print(f"[CameraThread] Trying camera with device={device}, api={api}")
                    self.cap = cv2.VideoCapture(device, api)
                    if self.cap.isOpened():
                        # Configure camera settings
                        self.cap.set(cv2.CAP_PROP_CONVERT_RGB, 1)
                        self.cap.set(cv2.CAP_PROP_BUFFERSIZE, 1)
                        self.cap.set(cv2.CAP_PROP_EXPOSURE, -3)  # Auto exposure
                        self.cap.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc(*'MJPG'))
                        self.cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
                        self.cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)
                        self.cap.set(cv2.CAP_PROP_FPS, 30)
                        ...
                        ret, _ = self.cap.read()
                        if ret:
                            print(f"[CameraThread] Successfully initialized camera {device} with {actual_width}x{actual_height} @ {actual_fps}fps")
                            success = True
                            break
                        else:
                            print(f"[CameraThread] Could not read test frame from camera {device}")
                            self.cap.release()
                except Exception as e:
                    print(f"[CameraThread] Error trying camera {device}: {str(e)}")
                    if self.cap:
                        self.cap.release()
                        self.cap = None
            
            if not success:
                print("[CameraThread] Failed to initialize any camera")
                self.cap = None
                
        except Exception as e:
            print(f"[CameraThread] Error in camera initialization: {str(e)}")
            if self.cap:
                self.cap.release()
            self.cap = None
        ...
        if self.cap and self.cap.isOpened():
            ...
            self.cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)
            self.cap.set(cv2.CAP_PROP_FPS, 30)
            print("[CameraThread] Camera initialized successfully with V4L2 backend.")
        else:
            print("[CameraThread] Error: Could not open camera with V4L2 backend. Please check camera connection, permissions, and that /dev/video0 exists.")
            self.cap = None
        ...
        try:
            ...
            import torch
            from ultralytics import YOLO
            print("[CameraThread] Initializing YOLO detection...")
            
            # Set up PyTorch
            torch.set_num_threads(4)  # Limit CPU threads
            device = torch.device('cpu')
            print("[CameraThread] PyTorch version:", torch.__version__)
            print("[CameraThread] Using device:", device)
            
            # Define model path and check if file exists
            model_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'yolov8n.pt')
            if not os.path.exists(model_path):
                print(f"[CameraThread] Error: Model file not found at {model_path}")
                self.model_detect = None
                return
                
            file_size = os.path.getsize(model_path)
            if file_size < 1000:  # File should be several MB
                print(f"[CameraThread] Error: Model file appears to be corrupted (size: {file_size} bytes)")
                self.model_detect = None
                return
            print(f"[CameraThread] Found model file: {file_size/1024/1024:.1f} MB")
            
            file_size = os.path.getsize(model_path)
            print(f"[CameraThread] Model file size: {file_size/1024/1024:.2f} MB")
            
            # Load the model
            print("[CameraThread] Loading YOLO model...")
            try:
                self.model_detect = YOLO(model_path)
                self.model_detect.to(device)
                
                # Verify model with a test prediction
                print("[CameraThread] Verifying model with test prediction...")
                dummy_image = np.zeros((100, 100, 3), dtype=np.uint8)
                with torch.no_grad():
                    results = self.model_detect.predict(source=dummy_image, verbose=False)
                    if not results:
                        raise ValueError("Model returned no results on test image")
                print("[CameraThread] YOLO model loaded and verified successfully")
            except Exception as e:
                print(f"[CameraThread] Error loading/verifying model: {str(e)}")
                import traceback
                traceback.print_exc()
                self.model_detect = None
                return
            ...
            print("[CameraThread] Error loading YOLO model:", str(e))
            self.model_detect = None

    def check_proximity_and_objects(self, frame):
        """Modified check that assumes person is present but still detects objects"""
        ...
        if self.model_detect is None:
            # Skip prediction if model isn't loaded
            return True, "person_only", None, None
            
        try:
            import torch  # Import torch for prediction
            with torch.no_grad():  # Disable gradient calculation for inference
                ...
                if not results:
                    print("[CameraThread] Warning: Model prediction returned empty results")
                    return True, "person_only", None, None
        except Exception as e:
            print(f"[CameraThread] Prediction error: {str(e)}")
            import traceback
            traceback.print_exc()
            return True, "person_only", None, None

        if not results or results[0] is None:
            print("[CameraThread] Error: Invalid prediction results")
            return True, "person_only", None, None
    ...
    def run(self):
        ...
        self.running = True
        if not USING_PI_CAMERA and self.cap is None:
            print("CameraThread: Camera not available, thread exiting.")
            self.running = False
            return
        if USING_PI_CAMERA and self.picam2 is None:
            print("CameraThread: Pi Camera not available, thread exiting.")
            self.running = False
            return

        print("CameraThread: Camera thread started.")
        consecutive_failures = 0
        max_failures = 20 # Try 20 times then exit
        while self.running:
            try:
                if USING_PI_CAMERA and self.picam2 is not None:
                    ...
                    if not ret:
                        raise Exception("Failed to read frame from OpenCV capture")
                else:
                    raise Exception("No camera interface available")
                ...
                print(f"Warning: Camera capture failed ({consecutive_failures}/{max_failures}): {str(e)}")
                self.msleep(100)
                if consecutive_failures >= max_failures:
                    print("CameraThread: Camera capture failed too many times. Exiting thread.")
                    self.running = False
                    break
                continue
                consecutive_failures += 1
                print(f"Warning: Could not read frame from camera. Retrying... ({consecutive_failures}/{max_failures})")
                self.msleep(100)
                if consecutive_failures >= max_failures:
                    print("CameraThread: Camera read failed too many times. Exiting thread.")
                    self.running = False
                    break
                continue
            consecutive_failures = 0
            ...
            # Modified logic - always in detection mode but still check for objects
            ...
            # Always stay in camera detection mode
            if not self.person_currently_detected:
                self.person_currently_detected = True
                self.proximity_changed.emit(True)
                print("CameraThread: Camera detection mode active.")

            # Check for objects to classify
            if detection_info == "objects_for_classification" and bbox:
                print("Object detected, preparing for classification...")
                ...
                if smart_bbox:
                    ...
                    print("Emitted object for classification")
    ...
    def stop(self):
        """Stop camera thread"""
        print("CameraThread: Stopping thread...")
        self.running = False
        ...
        if self.picam2:
            self.picam2.close()
            print("CameraThread: Pi Camera released.")
        print("CameraThread: Thread stopped.")
...
class WasteSorterApp(QMainWindow):
    ...
    def __init__(self):
        ...
        QTimer.singleShot(500, lambda: self.speak_text("Welcome to Sortyx! Your AI-powered smart bin is ready to help you sort your waste."))
    ...
    def on_classification_ready(self, classification, confidence, cropped_image, class_name_from_classifier, yolo_class_name=None, full_response=None, explanation=None):
        ...
        # --- TTS for classification result ---
        if final_classification not in ["Unknown", "Error", "No Waste Object"]:
            self.show_celebration_animation()
            if final_classification == "Recyclable":
                tts_msg = f"This is recyclable! Please put it in the green bin on your left. {message_text.replace('🌍','').strip()}"
            else:
                tts_msg = f"This is not recyclable. Please put it in the black bin on your right. {message_text.replace('❌','').strip()}"
            QTimer.singleShot(200, lambda: self.speak_text(tts_msg))
    ...
    def display_thank_you_message(self, title, message, duration_ms=4000):
        ...
        QTimer.singleShot(200, lambda: self.speak_text(f"{title}. {message}"))
    ...
    def speak_text(self, text):
        """Speak text using pyttsx3 in a non-blocking thread. Stops previous TTS if running."""
        self.stop_tts()
